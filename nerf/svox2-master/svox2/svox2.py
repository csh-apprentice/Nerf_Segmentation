...
class SparseGrid(nn.Module):
    ...
    def find_voxel(self, rays: Rays,return_raylen: bool=False):
        """
        Find the voxel given a point on the screen
        return: 
        """
        print("start to find the voxel of the click!")
        origins = self.world2grid(rays.origins)
        dirs = rays.dirs / torch.norm(rays.dirs, dim=-1, keepdim=True)
        viewdirs = dirs
        B = dirs.size(0)
        ...
        # initial guess on the link of the voxel
        ...
        while good_indices.numel() > 0:
            ...
            # color contribution voting strategy
            log_att = (
                -self.opt.step_size
                * torch.relu(sigma[..., 0])
                * delta_scale[good_indices]
